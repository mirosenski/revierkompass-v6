...
class PremiumExportService {
    ...
    async exportToPremiumExcel(routeResults, startAddress, allStations, customAddresses, options = {}) {
        try {
    ...
    prepareExportData(routeResults, allStations, customAddresses) {
    ...
    createMetaData(routeResults, startAddress) {
    ...
    createCorporateInfoSheet(metaData) {
    ...
    createStatisticsSheet(routeResults, allStations, customAddresses) {
        const cityStats = [];
        ...
        const ws = XLSX.utils.json_to_sheet(cityStats.sort((a, b) => parseFloat(a.Durchschnittliche_Entfernung_km) - parseFloat(b.Durchschnittliche_Entfernung_km)));
        ws['!cols'] = [
            { wch: 20 }, // Stadt
            { wch: 15 }, // Anzahl Ziele
    ...
    // CSV Export
    async exportToCSV(routeResults, allStations, customAddresses) {
        try {
    ...
    // Premium PDF Export
    async exportToPremiumPDF(routeResults, startAddress, allStations, customAddresses) {
        try {
            ...
            for (let i = 1; i <= totalPages; i++) {
                ...
                pdf.text(`Seite ${i} von ${totalPages} - RevierKompass v2.0 © Polizei Baden-Württemberg`, pageWidth / 2, pageHeight - 10, { align: 'center' });
            }
            // Save PDF
            const timestamp = new Date().toISOString().split('T')[0];
    ...
    // Copy to clipboard
    async copyToClipboard(routeResults, allStations, customAddresses) {
        try {
            const data = this.prepareExportData(routeResults, allStations, customAddresses);
