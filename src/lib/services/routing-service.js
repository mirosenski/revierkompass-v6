class RoutingService {
    ...
    routeCache = new Map();
    MAX_CACHE_SIZE = 100;
    buildOSRMUrl(start, end, baseUrl) {
        if (baseUrl) {
    ...
    async calculateMultipleRoutes(startAddress, selectedStationIds, selectedCustomAddressIds, allStations, customAddresses) {
        const results = [];
        ...
        for (const stationId of selectedStationIds) {
            ...
            if (!station)
                continue;
            try {
                const route = await this.calculateSingleRoute(startAddress.coordinates, station.coordinates);
                if (route) {
                    results.push({
                ...
                const directDistance = this.calculateDirectDistance(startAddress.coordinates, station.coordinates);
                results.push({
                    id: `station-${station.id}`,
                    destinationId: station.id,
                    destinationName: station.name,
        ...
        for (const addressId of selectedCustomAddressIds) {
            ...
            if (!customAddress || !customAddress.coordinates)
                continue;
            try {
                const route = await this.calculateSingleRoute(startAddress.coordinates, customAddress.coordinates);
                if (route) {
                    results.push({
                ...
                // Fallback to direct distance calculation
                const directDistance = this.calculateDirectDistance(startAddress.coordinates, customAddress.coordinates);
                results.push({
                    id: `custom-${customAddress.id}`,
                    destinationId: customAddress.id,
                    destinationName: customAddress.name,
    ...
    async calculateSingleRoute(start, end) {
    ...
    async calculateWithOSRM(request, baseUrl) {
    ...
    async calculateWithValhalla(request) {
        const { start, end } = request;
        const payload = {
            locations: [
