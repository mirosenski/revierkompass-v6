...
const AdminAddressManagement = () => {
    ...
    useEffect(() => {
        ...
        if (filters.search) {
            ...
            filtered = filtered.filter(addr => addr.name.toLowerCase().includes(searchLower) ||
    ...
    const handleSave = async (formData) => {
        ...
        try {
            if (editingAddress) {
                await adminAddressService.updateAddress(editingAddress.id, formData);
                toast.success('Adresse erfolgreich aktualisiert');
            }
            else {
                await adminAddressService.createAddress(formData);
    ...
    return (_jsxs("div", { className: "min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800", children: [_jsx("div", { className: "sticky top-0 z-40 bg-white/90 dark:bg-gray-800/90 backdrop-blur-md shadow-sm border-b border-gray-200/50 dark:border-gray-700/50", children: _jsx("div", { className: "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6", children: _jsxs("div", { className: "flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-3xl font-bold text-gray-900 dark:text-white", children: "Adressen verwalten" }), _jsxs("p", { className: "text-gray-600 dark:text-gray-400 mt-1", children: [filteredAddresses.length, " von ", addresses.length, " Adressen"] })] }), _jsxs("div", { className: "flex items-center gap-4", children: [_jsxs("label", { className: "flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer", children: [_jsx("input", { type: "checkbox", checked: filters.showInactive, onChange: (e) => handleFilterChange('showInactive', e.target.checked), className: "w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 focus:ring-offset-0" }), _jsx("span", { children: "Inaktive anzeigen" })] }), _jsxs("button", { onClick: handleCreateAddress, className: "flex items-center gap-2 px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl transition-all duration-200 shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2", children: [_jsx(Plus, { className: "w-5 h-5" }), _jsx("span", { className: "font-medium", children: "Neue Adresse" })] })] })] }) }) }), _jsx("div", { className: "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6", children: _jsx(AddressFilters, { filters: filters, onFilterChange: handleFilterChange, onClearFilters: clearFilters, allCities: allCities, hasActiveFilters: hasActiveFilters, filteredAddressesCount: filteredAddresses.length }) }), _jsx("div", { className: "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8", children: addresses.length === 0 ? (_jsxs("div", { className: "text-center py-12", children: [_jsx("div", { className: "bg-gray-100 dark:bg-gray-800 rounded-full p-4 w-20 h-20 mx-auto mb-4", children: _jsx(MapPin, { className: "w-12 h-12 text-gray-400 mx-auto" }) }), _jsx("h3", { className: "text-lg font-medium text-gray-900 dark:text-white mb-2", children: "Keine Adressen gefunden" }), _jsx("p", { className: "text-gray-600 dark:text-gray-400 mb-4", children: hasActiveFilters
                                ? 'Versuchen Sie, die Filter anzupassen oder zu löschen.'
                                : 'Erstellen Sie Ihre erste Adresse.' }), hasActiveFilters ? (_jsx("button", { onClick: clearFilters, className: "px-4 py-2 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg transition-colors", children: "Filter l\u00F6schen" })) : (_jsx("button", { onClick: handleCreateAddress, className: "px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors", children: "Erste Adresse erstellen" }))] })) : (_jsx("div", { className: "grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6", children: filteredAddresses.map((address) => (_jsx("div", { className: "animate-in fade-in-50 duration-200", children: _jsx(AddressCard, { address: address, onEdit: handleEditAddress, onDelete: handleDelete, onApprove: handleApprove, onReject: handleReject }) }, address.id))) })) }), _jsx(AddressModal, { address: editingAddress, isOpen: isModalOpen, onClose: handleCloseModal, onSave: handleSave, isLoading: isLoading, error: error, availablePraesidien: availablePraesidien })] }));
};
