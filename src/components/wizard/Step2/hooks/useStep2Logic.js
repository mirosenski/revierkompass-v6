...
export const useStep2Logic = () => {
    ...
    const [expandedPraesidien, setExpandedPraesidien] = useState(new Set());
    ...
    const fetchRoutes = async () => {
        ...
        try {
            ...
            for (let i = 0; i < allDestinations.length; i += batchSize) {
                ...
                const batchRoutes = await Promise.all(batch.map(async (dest) => {
                    ...
                    try {
                        ...
                        const response = await fetch(`https://router.project-osrm.org/route/v1/driving/${startCoords.lng},${startCoords.lat};${dest.coordinates.lng},${dest.coordinates.lat}?overview=full&geometries=geojson`, {
                ...
                const validRoutes = batchRoutes.filter(route => route !== null);
    ...
    const togglePraesidiumWithReviere = (praesidiumId) => {
        const praesidien = getStationsByType('praesidium');
    ...
    const togglePraesidiumExpansion = (praesidiumId) => {
    ...
    const handleCustomToggle = (addressId) => {
    ...
    const handleCommand = (command) => {
        ...
        if (command.startsWith('selectPraesidium:')) {
            ...
            const stuttgartPraesidien = praesidiumWithReviere.filter(p => p.city.toLowerCase().includes('stuttgart'));
